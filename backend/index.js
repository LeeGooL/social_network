...
const receiver = [bodyParser.json(), bodyParser.urlencoded({ extended: false }), upload.none()];
...
app.get('/api/user/:userId', async (req, res) => {
  try {
    ...
    posts = await Promise.all(
      posts.map((post) =>
        Post.likeDefine(post.id, req.session?.user?.id).then((data) => ({
          ...post,
          ...data,
        })),
      ),
    ...
    posts = await Promise.all(posts.map((post) => User.findByPk(post.userId).then((user) => ({ ...post, user }))));
...
app.post('/api/post', authenticated, receiver, async (req, res) => {
  try {
    if (req.session.user.id !== req.body.userId) {
      const isFriends = await Friend.isFriends(req.session.user.id, req.body.userId);
    ...
    const post = await Post.create(req.session.user.id, req.body.userId, req.body.content);
...
app.patch('/api/user/avatar', authenticated, upload.single('avatar'), async (req, res) => {
  try {
    if (!req.file) {
...
chatIo.on('connection', async (socket) => {
  ...
  const datas = await Promise.all(
    userChats.map(async (userChat) => {
      const message = await Message.getLastMessage(userChat, session.user.id);
  ...
  socket.on('getChat', async (friendId, callback) => {
    const [messages, friend] = await Promise.all([Message.getChat(session.user.id, friendId), User.findByPk(friendId)]);
